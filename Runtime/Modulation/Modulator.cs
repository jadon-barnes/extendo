...
namespace Extendo.Modulation
{
	public abstract class Modulator<T>
	{
		...
		{
			return GetValue(sine, time, from, to, cutoffFrom, cutoffTo);
		}

		protected T GetCosine(float time)
		{
			return GetValue(cosine, time, from, to, cutoffFrom, cutoffTo);
		}

		protected T GetLinear(float time)
		{
			return GetValue(linear, time, from, to, cutoffFrom, cutoffTo);
		}

		protected T GetPerlinNoise(float time)
		{
			return GetValue(perlinNoise, time, from, to, cutoffFrom, cutoffTo);
		}

		protected T GetBounce(float time)
		{
			return GetValue(bounce, time, from, to, cutoffFrom, cutoffTo);
		}
	}
}